...
/*TODO;
Fix Buffer file loc so it will work on all computers
Pass param to change which world/floor is generated by genRooms()
*/
...
public class Map {
	...
	public Map(int ml, int mw, String w) {
		...
		rooms = new String[ml][mw][6]; // Creates 3D array to stores columns, rows for rooms, and a 6 row picture of
										// the room
	}
	...
	public void genRooms() { // Generates map of rooms for world one
		...
		try {
			roomInput = new BufferedReader(
					new FileReader(this.worldFile)); // LOC NEEDS FIXED; Sets buffer reader to read from text file that stores rooms
			for (int k = 0; k < 3; k++) {
				for (int j = 0; j < 3; j++) {
					for (int i = 0; i < 6; i++) {
						rooms[k][j][i] = roomInput.readLine(); // Sets each line in the 3d array equal to a line in the text file
					}
					roomInput.readLine(); // Skips a line after every room (I leave a blank space in the text file for readability)
				}
			}
	...
	public String actionMneu(boolean[] details, int x, int y) {
		...
		if (details[6]) {
			System.out.println("Type V to enter vent to next world");
		}
